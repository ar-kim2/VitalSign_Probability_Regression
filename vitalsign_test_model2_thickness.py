...
if __name__ == '__main__':
    ...
    save_dir_name = "vitalsign_thickness_0104_prob_02_input14_m1_epoch5000_addinput3"
    ...
    with torch.no_grad():
        ...
        for anchor_t, pos_t, neg_t, m_label, _, _, th_label, _ in test_data_loader:
            if use_gpu == True:
            ...
            if len(out_list) == 0:
                ...
                label = th_label.detach().cpu().numpy()
            else:
                out_list = np.concatenate([out_list, anc_out_t.detach().cpu().numpy()])
                label = np.concatenate([label, th_label.numpy()])
    ...
    for anchor, m_label, m_label2, m_label3, tb_label, st_label, th_label, _, th_label3, total_label in classifier_loader:
        ...
        test_loss = torch.mean(-(torch.sum(th_label3 * torch.log(pred_prob+0.000000000000001), dim=1)))

        rmse_loss = criterion(pred_prob, th_label3)

        top_p, top_class = pred_prob.topk(1, dim=1)

        for data_idx in range(top_class.shape[0]):
            if top_class[data_idx][0].item() == th_label[data_idx].item():
                equal_cnt += 1
            predict_result.append(top_class[data_idx][0].item())
            gt_list.append(th_label[data_idx].item())
            total_cnt += 1
    ...
    test_loss = criterion(pred_value, th_value)
    ...
    loss_list = ((pred_value - th_value)**2)**(1/2)
    ...
    for i in range(len(pred_value)):
        ...
        gt_value_list.append(th_value[i].item())
