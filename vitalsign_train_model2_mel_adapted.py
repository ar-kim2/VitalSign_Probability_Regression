...
from dataset2_adapted import ViatalSignDataset_triplet_mel_thickness
from dataset2_adapted import ViatalSignDataset_class_mel_thickness
from dataset2_adapted import ViatalSignDataset_regression_mel_thickness

from sklearn.manifold import TSNE
from sklearn.metrics import confusion_matrix

from online_triplet_loss.losses import batch_adapted_triplet_loss
...
if __name__ == '__main__':
    ...
    save_dir = "vitalsign_mel_1230_prob_005_input14_adapted_triplet_m1_l2"
    ...
    classify_path = os.path.join(path, './result/{}/classification_weight_data_2'.format(save_dir))
    classify_path2 = os.path.join(path, './result/{}/classification_weight_data2_2'.format(save_dir))

    regression_path = os.path.join(path, './result/{}/regression_weight_data_2'.format(save_dir))
    regression_path2 = os.path.join(path, './result/{}/regression_weight_data2_2'.format(save_dir))
    ...
    data_loader = DataLoader(train_dataset, batch_size=500, shuffle=True)

    test_data_len = len(ViatalSignDataset_triplet_mel_thickness(mode='test', cl=class_mode, model_mel=-1, model_thick=-1))
    test_data_loader = DataLoader(test_dataset, batch_size=500, shuffle=False)
    ...
    for epoch in range(epochs):
        ...
        for anchor, ml, _, _, _, _ in data_loader:
            ...
            loss = batch_adapted_triplet_loss(ml, anc_out, margin=1, lamda=1)
        ...
        if epoch % 10 == 0:
            with torch.no_grad():
                ...
                for anchor_t, ml_t, _, _, _, _ in test_data_loader:
                    ...
                    test_loss = batch_adapted_triplet_loss(ml_t, anc_out_t, margin=1, lamda=1)
            ...
            if mean_loss < best_loss:
                ...
                print("{} | Epoch: {}/{} - Loss: {:.4f} , Test Loss: {:.4f} (Save Model)".format(t_stamp, epoch + 1, epochs,
                                                                                            mean_loss, test_loss))
                ...
                t_stamp = time.time()
                t_stamp = time.ctime(t_stamp)
                print("{} | Epoch: {}/{} - Loss: {:.4f} , Test Loss: {:.4f}".format(t_stamp, epoch + 1, epochs, mean_loss,
                                                                               test_loss))
